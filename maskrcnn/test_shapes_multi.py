...
class DrugDataset(utils.Dataset):
    ...
    def load_shapes(self, count, height, width, img_floder, mask_floder, imglist, dataset_root_path):
        ...
        """
        # Add classes
        self.add_class("shapes", 1, "needle1")
        self.add_class("shapes", 2, "needle2")
        self.add_class("shapes", 3, "needle3")
        self.add_class("shapes", 4, "needle4")
        self.add_class("shapes", 5, "needle5")
        self.add_class("shapes", 6, "needle6")
    ...
    def load_mask(self, image_id):
        ...
        for i in range(len(labels)):
            if labels[i].find("needle1") != -1:
                # print "box"
                labels_form.append("needle1")
            elif labels[i].find("needle2") != -1:
                 #print "column"
                 labels_form.append("needle2")
            elif labels[i].find("needle3") != -1:
                # print "package"
                labels_form.append("needle3")
            elif labels[i].find("needle4") != -1:
                # print "fruit"
                labels_form.append("needle4")
            elif labels[i].find("needle5") != -1:
                # print "fruit"
                labels_form.append("needle5")
            elif labels[i].find("needle6") != -1:
                # print "fruit"
                labels_form.append("needle6")
...
# 获取保存的权重，或者手动指定目录位置
model_path = os.path.join(ROOT_DIR, "logs/shapes20180731T1638/mask_rcnn_shapes_0002.h5")
...
root_dir = '/media/zdyd/file/maskRCNN/resize/'
output_dir = '/media/zdyd/file/maskRCNN/test_output/'
#root_dir = '/media/zdyd/code/yuanfei/object_detection/Mask_RCNN/watch/rgb'
files = os.listdir(root_dir)
for file in files:
    t1 = time.clock()
    if file[-3:] == 'jpg':
        im_path = os.path.join(root_dir,file)
        ...
        '''
        retval, img = cv2.threshold(ret_img, 128, 1, cv2.THRESH_BINARY)
        skeleton = morphology.skeletonize(img)
        #print skeleton
        fig, (ax1) = plt.subplots(nrows=1, ncols=2, figsize=(8, 4))
        newImg = Image.fromarray(skeleton.astype(np.uint8))
        # newImg = newImg*255
        #newImg.save('test_1.png', 'PNG')
        
        
        #new_img = Image.fromarray(ret_img.astype('uint8'))
        #new_img.save(output_dir+file, 'JPEG')
        '''
...
'''
#read image to detect and segment
#im_path = '/media/zdyd/code/yuanfei/object_detection/Mask_RCNN/data_watch/rgb/rgb_40.jpg'
im_path = '/media/zdyd/code/yuanfei/object_detection/Mask_RCNN/press/rgb/rgb_17.jpg'
img = np.array(Image.open(im_path))
print(img[0][0])
results = model.detect([img], verbose=1) # 预测
#print(results[0]['masks']*255)
ret_img = results[0]['masks']*255
ret_img.resize((256,256))
#np.set_printoptions(threshold=np.inf)
#print(ret_img)
#ret_img = ret_img.astype(np.int32)
#print(type(ret_img))


new_img = Image.fromarray(ret_img.astype('uint8'))
new_img.save('./test.jpg','JPEG')
#r = results[0]
#visualize.display_instances(img, r['rois'], r['masks'], r['class_ids'],dataset_val.class_names, r['scores'], ax=get_ax())
#'''
'''
im = np.array(new_img)
retval, img = cv2.threshold(im, 128, 1, cv2.THRESH_BINARY)
skeleton = morphology.skeletonize(img)
#print skeleton
#fig, (ax1) = plt.subplots(nrows=1, ncols=2, figsize=(8, 4))
newImg = Image.fromarray(skeleton.astype(np.uint8))
...
for d in degree:
    ...
    print(clf.named_steps['linear'])
    b, k = clf.named_steps['linear'].coef_
    ratio = math.atan(k)
    print(ratio * 180 / 3.14)
