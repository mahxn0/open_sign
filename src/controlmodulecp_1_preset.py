...
def get_singleimage(str_planeinfo, ser_dev,ipstream_dev, data, single_image,singleid,status):
    print data
    ...
    for i in range(len(framecentral_x)):
        ...
        final_foucs = (captureimage_height / 2) * (ori_focus / 819.13 + 1) / (abs(frame_height)) * 3276;
		...
		if status==0:
			...
			goto_preset(ser_dev, cmd_gotopreset200)
        		time.sleep(5)
	#return locate ptz state
    return frame_stream

def get_singleimageori(str_planeinfo, ser_dev,ipstream_dev, data, single_image,singleid):
    print data
    print('#########get_singleimage#########')
    print str_planeinfo
    param_planeinfo = []
    ...
    for i in range(len(framecentral_x)):
        ...
        #返回预置点
        	...
        	time.sleep(5)
	#return locate ptz state
    return frame_stream,finalx_set,finaly_set,final_foucs


def get_deviceimage(str_planeinfo, ser_dev,ipstream_dev, data, single_image,singleid,prex_set,prey_set,prezoom_set):
    print data
    print('#########get_singleimage#########')
    print prex_set,prey_set,prezoom_set
    print str_planeinfo
    param_planeinfo = []
    param_planeinfo = str_planeinfo.split('/')
    camera_id_singleimage = int(param_planeinfo[2])
    ...
    for i in range(len(framecentral_x)):
        ...
        frame_height = data.end_y[i] - data.begin_y[i];
        final_foucs = (captureimage_height / 2) * (ori_focus / 819.13 + 1) / (abs(frame_height)) * 3276;
	final_foucs= 819.13*2*final_foucs/ori_focus
	print("final_focus:",final_foucs)
        if final_foucs > 16380:
            final_foucs = 16380
        print "final_foucs: " + str(int(final_foucs))
