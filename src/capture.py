									...
									if yolo_surface is not None :
										...
										if len(surfaceBox)>0:
											for box in surfaceBox:
												...
												mulmsg.header.frame_id = submsg+'/'+str(multiPicId)+'/'+str(subMultipId)+'-'+str(subSingleId)+'-'+box[0]
												record_pub.publish(mulmsg)
												img,prex,prey,lagerfacter,testid = ct.get_singleimage(submsg,ser_dev1,ipstream_dev1,msg,single_image,picId)
												...
												if len(singleBox)>0:
													for res in singleBox:
														...
														cv2.imwrite(SinboardPath+str(multiPicId)+'-'+str(subMultipId)+'-'+str(subSingleId)+'.jpg',single_img)
														sig=CompressedImage();
														sig.data = np.array(cv2.imencode('.jpg',single_img)[1]).tostring()
														sig.header.frame_id = submsg+'/'+str(multiPicId)+'/'+str(subMultipId)+'-'+str(subSingleId)+'-'+res[0]
														...
														if len(device_Box)>0:
															...
															for d_Box in device_Box:
																...
																cv2.imwrite(DevicePath+str(multiPicId)+'-'+str(subMultipId)+'-'+str(subSingleId)+'-'+d_Box[0]+'.jpg',single_img)
																msg = CompressedImage()
																msg.data = np.array(cv2.imencode('.jpg',single_img)[1]).tostring()
																msg.header.frame_id = submsg+'/'+str(multiPicId)+'/'+str(subMultipId)+'-'+str(subSingleId)+'-'+d_Box[0]
																record_pub.publish(msg)
																final_img,test1id=ct.get_deviceimage(submsg,ser_dev1,ipstream_dev1,dig,single_image,picId,prex,prey,lagerfacter)
																...
																if final_img !=None:
																	...
																	for f_box in final_Box:
																		if f_box[0]==ResultLable:
																			...
																			cv2.imwrite(DevicePath+str(multiPicId)+str(subMultipId)+str(subSingleId)+f_box[0]+'.jpg',device_img_rect)
																			...
																			global picId
																			multiPicId=picId
																			resultmsg.header.frame_id=submsg+'/'+str(multiPicId)+'/'+str(subMultipId)+'/'+str(subSingleId)+'-'+ResultLable
																			msg = CompressedImage()
																			msg.data = np.array(cv2.imencode('.jpg',device_img_rect)[1]).tostring()
																			msg.header.frame_id = submsg+'/'+str(multiPicId)+'/'+str(subMultipId)+'-'+str(subSingleId)+'-'+ResultLable
																			record_pub.publish(resultmsg)
																			single_image.publish(msg)
																			print ("%s Send Success", ResultLable)
