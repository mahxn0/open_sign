...
def callback_transferPub(data):
    ...
    if menuflag==0:
        ...
        if startTimeFlag is None:
            ...
            print('########3',startTimeFlag,taskId,camId,posId,camposId)
#    if menuflag=1:
#	#相机号/动作方向/动作步长
    ...
    while True:
        try:
		    ...
		    if ipstream_dev1.isOpened():
		    	...
		    	global picId
    		    	#cv2.imwrite('/home/nvidia/workspace/src/detectAndRecog/src/yolo_surface/src/record/'+str(picId)+'.jpg',image000)
                    	msg = CompressedImage()                                                                                                      
                    	msg.data = np.array(cv2.imencode('.jpg', img)[1]).tostring()                                                         
                    	msg.header.frame_id = submsg + '/'+str(picId)
		            ...
		            if True:
		                ...
		                if not captureQueue.empty():
							...
							subMultipId+=1
						    	...
						    	global picId
			    			        img = ct.get_singleimage(submsg,ser_dev1,ipstream_dev1,msg,single_image,picId,1)
							singleBox=yolo_surface.detect(img)
							single_img=img
							resList=[]
							global subSingleId
							subSingleId=0
							print("singleBox:",singleBox);
							if len(singleBox)>0:
								for res in singleBox:
									...
									device_img_rect=single_img_rect
									cv2.rectangle(single_img,(x1,y1),(x2,y2),(0,255,0),5)
									cv2.imwrite(SinboardPath+str(picId)+'-'+str(subSingleId)+'.jpg',single_img)
									global device_surface
									...
									if len(device_Box)>0:
										...
										for d_Box in device_Box:
											...
											dig.begin_x = [int(d_Box[2])]
			    	                        				dig.begin_y = [int(d_Box[3])]
			    		 	        				dig.end_x = [int(d_Box[4])]
			    			        				dig.end_y = [int(d_Box[5])]
											print("device location:",int(d_Box[2])+x1,int(d_Box[3])+y1,int(d_Box[4])+x1,int(d_Box[5])+y1)
			    			        				#cv2.rectangle(device_img_rect,(int(d_Box[2]),int(d_Box[3])),(int(d_Box[4]),int(d_Box[5])),(0,255,0),4)
											cv2.rectangle(single_img,(int(d_Box[2])+x1,int(d_Box[3])+y1),(int(d_Box[4])+x1,int(d_Box[5])+y1),(255,0,0),4)
											cv2.imwrite(str(subSingleId)+'.jpg',single_img)											
											print("@@@@@@@@@@@sinto Device detect")				    			    												
										
											final_img=ct.get_singleimage(submsg,ser_dev1,ipstream_dev1,dig,single_image,picId,0)
											 
											rsg = CompressedImage()
											rsg.data = np.array(cv2.imencode('.jpg',final_img)[1]).tostring()
											rsg.header.frame_id = submsg+'/'+str(picId)+'/'+str(subMultipId)+'-'+str(subSingleId)+'-'+ResultLable
											cv2.imwrite(DevicePath+str(picId)+'-'+str(subMultipId)+'-'+str(subSingleId)+'-'+str(ResultLable)+'.jpg',final_img)
											# /2018 12:12:12/posid/taskid/camid/campos/singleid/subMultip-subSingle-sign/watch
											single_image.publish(rsg)
											ResultLable=''	
											dig.task_flag = str(posId) + '/' + str(taskId) + '/' + str(camId) + '/' + str(camposId)  
			                            					ptz_pos.publish(dig)				
							    #resList.append(sig)
							    #cv2.rectangle(single_rect,(getXY(box[2]),getXY(box[3])),(getXY(box[4]),getXY(box[5])),(0,0,255),5)
			    			    	    #cv2.imwrite('sig'+str(multiPicId)+'.jpg',single_rect)
							    #for sig in resListL
								#single_image_list=ct.get_singleimage(submsg,ser_dev1,ipstream_dev1,sig,picId)
