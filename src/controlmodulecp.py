...
def get_singleimage(str_planeinfo, ser_dev,ipstream_dev, data, single_image,singleid):
	...
	#返回放大倍数
        ...
        #返回预置点
		# recordDir = '/home/nvidia/workspace/src/detectAndRecog/src/yolo_surface/data/output'
        #         files = os.listdir(recordDir)
        #         for name in files:
        #                 curIndex = name.split('.')[0]
        #                 #print('########curIndex',curIndex)
        #                 if int(curIndex) > int(numId):
        #                     numId = int(curIndex)
        	goto_preset(ser_dev, cmd_gotopreset)
        	time.sleep(5)
	#return locate ptz state
    return frame_stream,finalx_set,finaly_set,lagerfacter
