									...
									if yolo_surface is not None :
										...
										if len(surfaceBox)>0:
											for box in surfaceBox:
												...
												img,prex,prey,lagerfacter = ct.get_singleimage(submsg,ser_dev1,ipstream_dev1,msg,single_image,picId)
												...
												if len(singleBox)>0:
													for res in singleBox:
														...
														sig.header.frame_id = submsg+'/'+str(multiPicId)+'/'+str(subMultipId)+'-'+str(subSingleId)+'-'+ResultLable
														...
														if len(device_Box)>0:
															...
															for d_Box in device_Box:
																...
																cv2.imwrite(DevicePath+str(multiPicId)+'-'+str(subMultipId)+'-'+str(subSingleId)+'-'+ResultLable+'.jpg',single_img)
																msg = CompressedImage()
																...
																record_pub.publish(msg)
																final_img=ct.get_deviceimage(submsg,ser_dev1,ipstream_dev1,dig,single_image,picId,prex,prey,lagerfacter)
																...
																if final_img !=None:
																	...
																	for f_box in final_Box:
																		if f_box[0]==ResultLable:
																			...
																			cv2.imwrite(DevicePath+ResultLable+'.jpg',device_img_rect)
																			msg = CompressedImage()
																			msg.data = np.array(cv2.imencode('.jpg',device_img_rect)[1]).tostring()
																			msg.header.frame_id = submsg+'/'+str(multiPicId)+'/'+str(subMultipId)+'-'+str(subSingleId)+'-'+f_box[0]
